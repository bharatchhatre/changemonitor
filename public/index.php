        ...
        <div class="login-wrapper">
            <div class="login-card">
                ...
                <?php if (!empty($loginError)): ?>
                    <div
                        style="background: var(--danger-bg); color: var(--danger); padding: 0.75rem 1rem; border-radius: var(--radius-sm); font-size: 0.875rem; margin-bottom: 1.25rem; border: 1px solid rgba(239, 68, 68, 0.3);">
                ...
                <form method="POST" action="">
                    ...
                    <div class="form-group"
                        style="margin-top: 1rem; display: flex; align-items: center; gap: 0.5rem; user-select: none;">
                        <input type="checkbox" id="remember" name="remember" class="custom-checkbox" value="1" checked>
                        <label for="remember"
                            style="font-size: 0.875rem; color: var(--text-secondary); cursor: pointer;">Keep me logged in on
                            this browser</label>
                    </div>
                    <button type="submit" class="btn btn-primary"
                        style="width: 100%; margin-top: 1.25rem; min-height: 44px;">Unlock Dashboard</button>
                </form>
            </div>
        </div>
    </body>
...
<body>
    <div class="app-container">
        ...
        <header class="app-header">
            <div class="brand-area" style="display: flex; align-items: center; gap: 0.75rem;">
                <div class="brand-icon">⚡</div>
                <div>
                    <div class="brand-title"><?= htmlspecialchars(cm_env('APP_NAME', 'Website Change Monitor')) ?></div>
                    <div class="brand-subtitle">bharatchhatre &bull; v<?= CM_VERSION ?></div>
                </div>
            </div>
            <div class="nav-actions">
                <button class="drawer-toggle-btn" id="mobileDrawerOpenBtn" aria-label="Open Navigation Drawer"
                    title="Open Menu">
                    ☰ <span style="font-size: 0.85rem; font-weight: 600;">Menu</span>
                </button>
                <a href="?route=logout" class="btn btn-secondary btn-sm desktop-logout-btn" title="Logout">Logout</a>
            </div>
        </header>

        <!-- Navigation Tabs (Desktop & Mobile Sync) -->
        <nav class="tabs-nav" id="desktopNavTabs">
        ...
        <div class="drawer-backdrop" id="mobileDrawerBackdrop">
            <div class="drawer-content">
                <div class="drawer-header">
                    <div style="display: flex; align-items: center; gap: 0.6rem;">
                        <div class="brand-icon" style="width: 32px; height: 32px; font-size: 1rem;">⚡</div>
                        <span style="font-weight: 700; font-size: 1.05rem;">Navigation</span>
                    </div>
                    <button type="button" class="drawer-close-btn" id="mobileDrawerCloseBtn">&times;</button>
                </div>
                <div class="drawer-menu">
                    <button class="drawer-item active" data-tab="tab-monitors">
                        <span>🎯 Monitors</span>
                        <span class="badge"
                            style="background: var(--accent-light); color: var(--text-primary); font-size: 0.75rem; padding: 0.15rem 0.5rem; border-radius: 12px;"><?= count($monitors) ?></span>
                    </button>
                    <button class="drawer-item" data-tab="tab-analytics">
                        <span>📊 Analytics</span>
                    </button>
                    <button class="drawer-item" data-tab="tab-logs">
                        <span>📋 Error & System Logs</span>
                    </button>
                    <button class="drawer-item" data-tab="tab-templates">
                        <span>🛡️ Request Profiles</span>
                    </button>
                    <button class="drawer-item" data-tab="tab-settings">
                        <span>⚙️ Settings & Notifications</span>
                    </button>
                    <hr style="border: 0; border-top: 1px solid var(--border-color); margin: 0.75rem 0;">
                    <a href="?route=logout" class="drawer-item" style="color: var(--danger);">
                        <span>🚪 Logout</span>
                    </a>
                </div>
            </div>
        ...
        <div class="stats-grid">
            <div class="stat-card stat-card-clickable" id="statCardTargets" title="Click to view Active Monitors"
                style="cursor: pointer;">
                <div class="stat-label">Total Targets 🎯</div>
                <div class="stat-value"><?= count($monitors) ?></div>
                <div class="stat-desc"><?= $activeCount ?> active monitoring (click to view)</div>
            </div>
            <div class="stat-card stat-success stat-card-clickable" id="statCardChecks"
                title="Click to view Check Activity Analytics" style="cursor: pointer;">
                <div class="stat-label">Total Checks 📊</div>
                <div class="stat-value"><?= number_format($stats['total_checks'] ?? 0) ?></div>
                <div class="stat-desc">Avg speed: <?= $stats['avg_response_time_ms'] ?? 0 ?>ms (click to view analytics)
                </div>
            </div>
            <div class="stat-card stat-warning stat-card-clickable" id="statCardChanges"
                title="Click to view all detected changes" style="cursor: pointer;">
            ...
            <div class="stat-card stat-danger stat-card-clickable" id="statCardErrors" title="Click to view error log"
                style="cursor: pointer;">
                <div class="stat-label" style="display: flex; justify-content: space-between; align-items: center;">
                    <span>Errors Encountered ⚠️</span>
                    <span class="unread-badge" id="unreadBadgeErrors" style="display: none;">NEW</span>
                </div>
        ...
        <div class="bulk-action-bar" id="bulkActionBar">
            ...
            <div class="bulk-btn-group">
                <button type="button" class="btn btn-primary btn-sm" id="bulkEditBtn"
                    title="Bulk edit settings for selected">✏️ Bulk Edit</button>
                <button type="button" class="btn btn-success btn-sm" id="bulkActivateBtn"
                    title="Set selected to Active">▶️ Activate</button>
                <button type="button" class="btn btn-secondary btn-sm" id="bulkPauseBtn"
                    title="Set selected to Paused">⏸️ Pause</button>
                <button type="button" class="btn btn-secondary btn-sm" id="bulkGroupBtn"
                    title="Assign selected to Group">📁 Set Group</button>
                <button type="button" class="btn btn-secondary btn-sm" id="bulkRunBtn"
                    title="Execute checks for selected">⚡ Run Now</button>
                <button type="button" class="btn btn-danger btn-sm" id="bulkDeleteBtn"
                    title="Move selected to Trash">🗑️ Delete</button>
                <button type="button" class="btn btn-secondary btn-sm" id="bulkDeselectBtn"
                    style="padding: 0.35rem 0.6rem;" title="Clear selection">&times; Clear</button>
            </div>
        </div>
        ...
        <div class="tab-pane" id="tab-monitors">
            <div class="panel">
                <div class="panel-header" style="flex-wrap: wrap; gap: 0.75rem; align-items: center;">
                    <div>
                        <div class="panel-title">Monitored Targets</div>
                        <span style="font-size: 0.85rem; color: var(--text-muted);">Auto-refreshed on manual or cron
                            check</span>
                    </div>

                    <!-- Monitors Primary Action Buttons -->
                ...
                <div class="group-filter-bar" id="groupFilterBar">
                    <span
                        style="font-size: 0.8rem; font-weight: 600; color: var(--text-muted); display: inline-flex; align-items: center; gap: 0.25rem;">
                        📁 Group:
                    </span>
                    ...
                    <?php foreach ($groups as $grpName => $grpCount): ?>
                        <?php if (strcasecmp($grpName, 'ungrouped') === 0)
                            continue; ?>
                        <button type="button" class="group-pill" data-group="<?= htmlspecialchars($grpName) ?>">
                            <?= htmlspecialchars($grpName) ?> (<span class="group-count"
                                data-count-group="<?= htmlspecialchars($grpName) ?>">0</span>)
                        </button>
                    <?php endforeach; ?>
                </div>
                            ...
                            <thead>
                                <tr>
                                    <th class="th-checkbox">
                                        <input type="checkbox" class="custom-checkbox select-all-checkbox"
                                            data-target="active" title="Select all active targets">
                                    </th>
                                    <th>Status</th>
                                    <th>Target / Name</th>
                                    <th>Group</th>
                                ...
                                <?php if (empty($activeMonitors)): ?>
                                    <tr class="empty-state-row">
                                        <td colspan="10"
                                            ...
                                            No active targets. Click <strong>"+ Add Target"</strong> or <strong>"➕ Bulk
                                                Add"</strong> above or resume a paused monitor from the Inactive sub-tab.
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($activeMonitors as $id => $m): ?>
                                        ...
                                        <tr data-id="<?= $id ?>" data-group="<?= htmlspecialchars($grp) ?>" class="monitor-row">
                                            <td class="td-checkbox">
                                                <input type="checkbox" class="custom-checkbox monitor-checkbox"
                                                    value="<?= $id ?>" data-status="active"
                                                    aria-label="Select monitor <?= htmlspecialchars($m['name']) ?>">
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <div
                                                    style="font-size: 0.775rem; color: var(--text-muted); word-break: break-all;">
                                                    <a href="<?= htmlspecialchars($m['url']) ?>" target="_blank"
                                                        rel="noopener noreferrer"><?= htmlspecialchars($m['url']) ?></a>
                                                </div>
                                            </td>
                                            <td>
                                                <span class="badge badge-group" title="Category Group">📁
                                                    <?= htmlspecialchars($grp) ?></span>
                                            </td>
                                            <td>
                                                <span
                                                    class="badge badge-type"><?= htmlspecialchars($m['type'] ?? 'html_full') ?></span>
                                            ...
                                            <td>
                                                <div class="next-check-timer" data-next-timestamp="<?= $nextCheckTs ?>"
                                                    data-status="active"
                                                    style="font-family: var(--font-mono); font-size: 0.85rem; color: var(--info); font-weight: 600;">
                                                    <?= $isOverdue ? '⚡ Due Now' : '⏳ in ' . round(($nextCheckTs - time()) / 60) . 'm' ?>
                                                </div>
                                                <div style="font-size: 0.725rem; color: var(--text-muted);">
                                                    Every
                                                    <?= $intervalMins ?>m<?= !empty($m['peak_schedule_enabled']) ? ' (dynamic)' : '' ?>
                                                </div>
                                            </td>
                                            <td>
                                                <span onclick="openChangesExplorer('<?= $id ?>')" style="cursor: pointer;"
                                                    title="Click to view detected changes for this target">
                                                    <strong
                                                        class="badge <?= ((int) ($m['change_count'] ?? 0) > 0) ? 'badge-changed' : '' ?>"><?= (int) ($m['change_count'] ?? 0) ?></strong>
                                                </span>
                                            ...
                                            <td style="text-align: right; white-space: nowrap;">
                                                <button class="btn btn-secondary btn-sm"
                                                    onclick="toggleStatus('<?= $id ?>', this)" title="Pause / Disable Monitor"
                                                    style="color: var(--warning);">⏸️</button>
                                                <button class="btn btn-secondary btn-sm" onclick="runCheck('<?= $id ?>', this)"
                                                    title="Run Check Now">⚡</button>
                                                <button class="btn btn-secondary btn-sm" onclick="viewHistory('<?= $id ?>')"
                                                    title="View History & Snapshots">📜</button>
                                                <button class="btn btn-secondary btn-sm"
                                                    onclick='editMonitor(<?= json_encode($m, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
                                                    title="Edit Monitor">✏️</button>
                                                <button class="btn btn-danger btn-sm"
                                                    onclick="deleteMonitor('<?= $id ?>', '<?= htmlspecialchars(addslashes($m['name'])) ?>')"
                        ...
                        <table class="data-table">
                            <thead>
                                <tr>
                                    <th class="th-checkbox">
                                        <input type="checkbox" class="custom-checkbox select-all-checkbox"
                                            data-target="inactive" title="Select all inactive targets">
                                    </th>
                                    <th>Status</th>
                                    <th>Target / Name</th>
                                    <th>Group</th>
                                ...
                                <?php else: ?>
                                    <?php foreach ($inactiveMonitors as $id => $m): ?>
                                        ...
                                        <tr data-id="<?= $id ?>" data-group="<?= htmlspecialchars($grp) ?>" class="monitor-row">
                                            ...
                                            <td style="text-align: right; white-space: nowrap;">
                                                <button class="btn btn-secondary btn-sm"
                                                    onclick="toggleStatus('<?= $id ?>', this)" title="Resume / Enable Monitor"
                                                    style="color: var(--success);">▶️ Resume</button>
                                                <button class="btn btn-secondary btn-sm" onclick="runCheck('<?= $id ?>', this)"
                                                    title="Run Check Now">⚡</button>
                                                <button class="btn btn-secondary btn-sm" onclick="viewHistory('<?= $id ?>')"
                                                    title="View History & Snapshots">📜</button>
                ...
                <div class="subtab-pane" id="subtab-trash" style="display: none;">
                    <div
                        style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem; padding: 0.75rem 1rem; background: var(--bg-subtle); border-radius: var(--radius-sm); border: 1px solid var(--border-color);">
                        <div>
                            <strong style="color: var(--danger);">🗑️ Deleted Targets Bin</strong>
                            <span style="font-size: 0.825rem; color: var(--text-muted); margin-left: 0.5rem;">Deleted
                                monitors are kept here. Restore anytime or permanently purge.</span>
                        </div>
                        <div style="display: flex; gap: 0.5rem;">
                            <?php if (!empty($trash)): ?>
                                <button type="button" class="btn btn-secondary btn-sm" id="bulkRestoreTrashBtn">♻️ Restore
                                    Selected</button>
                                <button type="button" class="btn btn-danger btn-sm" id="emptyTrashBtn">🔥 Empty
                                    Trash</button>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="data-table">
                            <thead>
                                <tr>
                                    <th class="th-checkbox">
                                        <input type="checkbox" class="custom-checkbox select-all-trash-checkbox"
                                            title="Select all trash items">
                                    </th>
                                    <th>Deleted At</th>
                                    <th>Target / Name</th>
                                    <th>Group</th>
                                ...
                                <?php else: ?>
                                    <?php foreach ($trash as $id => $m): ?>
                                        ...
                                        <tr data-id="<?= $id ?>" class="trash-row">
                                            <td class="td-checkbox">
                                                <input type="checkbox" class="custom-checkbox trash-checkbox" value="<?= $id ?>"
                                                    aria-label="Select deleted target <?= htmlspecialchars($m['name'] ?? '') ?>">
                                            </td>
                                            <td>
                                            ...
                                            <td style="text-align: right; white-space: nowrap;">
                                                <button class="btn btn-primary btn-sm"
                                                    onclick="restoreMonitor('<?= $id ?>', '<?= htmlspecialchars(addslashes($m['name'] ?? '')) ?>')"
                                                    title="Restore to Active Monitors">♻️ Restore</button>
                                                <button class="btn btn-danger btn-sm"
                                                    onclick="purgeDeletedMonitor('<?= $id ?>', '<?= htmlspecialchars(addslashes($m['name'] ?? '')) ?>')"
                                                    title="Delete Forever">🗑️ Purge</button>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            ...
                            if (empty($checksByDate)):
                                ...
                                <tr>
                                    <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 2rem;">No
                                        analytics logged yet. Run some checks to populate data.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($checksByDate as $date => $row): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $errCount > 0 ? "<span class='badge badge-error'>{$errCount}</span>" : '0' ?>
                                        </td>
                                        <td><span class="badge badge-active"><?= $rate ?>%</span></td>
                                        <td style="text-align: right; white-space: nowrap;">
                                            <button type="button" class="btn btn-secondary btn-sm"
                                                onclick="openChangesForDate('<?= htmlspecialchars($date) ?>')"
                                                title="View detected changes for <?= htmlspecialchars($date) ?>"
                                                style="padding: 0.25rem 0.6rem; font-size: 0.775rem;">
                                                🔍 Changes (<?= $chgCount ?>)
                                            </button>
                                            <button type="button" class="btn btn-secondary btn-sm"
                                                onclick="openLogsForDate('<?= htmlspecialchars($date) ?>')"
                                                title="View logs & errors for <?= htmlspecialchars($date) ?>"
                                                style="padding: 0.25rem 0.6rem; font-size: 0.775rem; margin-left: 4px;">
                                                ⚠️ Logs (<?= $errCount ?>)
                                            </button>
                                        </td>
                                    </tr>
            ...
            <div class="panel">
                <div class="panel-header">
                    ...
                    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                        ...
                        <button class="btn btn-secondary btn-sm" id="deleteSelectedLogsBtn" style="display: none;">🗑️
                            Delete Selected (<span id="selectedLogsCount">0</span>)</button>
                ...
                <!-- Search & Filters Bar -->
                <div
                    style="display: flex; gap: 0.75rem; flex-wrap: wrap; margin-bottom: 1.25rem; background: var(--bg-subtle); padding: 1rem; border-radius: var(--radius-md); border: 1px solid var(--border-color);">
                    <div style="flex: 1; min-width: 200px;">
                        <input type="text" id="logSearchInput" class="form-control"
                            placeholder="🔍 Search message, error or monitor ID..." style="min-height: 38px;">
                    </div>
                    <div style="width: 150px;">
                        <input type="date" id="logDateInput" class="form-control" title="Filter logs by date"
                        ...
                        <thead>
                            <tr>
                                <th style="width: 40px;" class="th-checkbox">
                                    <input type="checkbox" id="selectAllLogsCheckbox" class="custom-checkbox"
        ...
        <div class="tab-pane" id="tab-templates" style="display: none;">
            <div class="panel">
                ...
                <p style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 1.5rem;">
                    These profiles send complete modern browser request headers (including Sec-Ch-Ua, Accept-Language,
                    and Keep-Alive) to prevent cloud firewalls and anti-scraping filters from blocking requests.
                </p>

                <div class="form-grid">
                    <?php foreach ($builtinTemplates as $key => $tmpl): ?>
                        <div
                            style="background: var(--bg-subtle); padding: 1.25rem; border-radius: var(--radius-md); border: 1px solid var(--border-color);">
                            <div
                            ...
                            <div
                                style="font-size: 0.775rem; color: var(--text-muted); font-family: var(--font-mono); word-break: break-all; margin-bottom: 0.75rem;">
            ...
            <div class="panel">
                ...
                <form id="settingsForm">
                    ...
                    <div style="margin-bottom: 2rem;">
                        <h3
                            style="font-size: 1rem; color: var(--info); margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between;">
                            <span>✈️ Telegram Bot Alerts</span>
                            <button type="button" class="btn btn-secondary btn-sm test-notify-btn"
                                data-channel="telegram">Test Telegram</button>
                        </h3>
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label" for="settingTelegramToken">Bot API Token</label>
                                <input type="text" id="settingTelegramToken" class="form-control"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="settingTelegramChatId" class="form-control"
                    ...
                    <div style="margin-bottom: 2rem;">
                        <h3
                            style="font-size: 1rem; color: var(--success); margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between;">
                            <span>💬 OpenWA (WhatsApp Webhook) Alerts</span>
                            <button type="button" class="btn btn-secondary btn-sm test-notify-btn"
                                data-channel="openwa">Test OpenWA</button>
                        </h3>
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label" for="settingOpenwaUrl">OpenWA Server / Webhook URL</label>
                                <input type="text" id="settingOpenwaUrl" class="form-control"
                                    placeholder="http://your-server:8080/sendText"
                                    value="<?= htmlspecialchars($settings['openwa_api_url'] ?? '') ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingOpenwaKey">API Key / Token (Optional)</label>
                                <input type="text" id="settingOpenwaKey" class="form-control"
                                    placeholder="Bearer or API key..."
                                    value="<?= htmlspecialchars($settings['openwa_api_key'] ?? '') ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingOpenwaChatId">WhatsApp Number / Group ID</label>
                                <input type="text" id="settingOpenwaChatId" class="form-control"
                                    placeholder="1234567890 (no + sign or @c.us)"
                                    value="<?= htmlspecialchars($settings['openwa_chat_id'] ?? '') ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 2rem;">
                        <h3
                            style="font-size: 1rem; color: var(--warning); margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between;">
                            <span>📧 Gmail SMTP Email Alerts</span>
                            <button type="button" class="btn btn-secondary btn-sm test-notify-btn"
                                data-channel="email">Test Email</button>
                        </h3>
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label" for="settingSmtpHost">SMTP Host</label>
                                <input type="text" id="settingSmtpHost" class="form-control"
                                    value="<?= htmlspecialchars($settings['gmail_smtp_host'] ?? 'smtp.gmail.com') ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingSmtpPort">SMTP Port</label>
                                <input type="number" id="settingSmtpPort" class="form-control"
                                    value="<?= (int) ($settings['gmail_smtp_port'] ?? 587) ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingSmtpUser">Gmail Address (Sender)</label>
                                <input type="email" id="settingSmtpUser" class="form-control"
                                    placeholder="youremail@gmail.com"
                                    value="<?= htmlspecialchars($settings['gmail_smtp_user'] ?? '') ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingSmtpPass">Google App Password (16 chars)</label>
                                <input type="password" id="settingSmtpPass" class="form-control"
                                    placeholder="xxxx xxxx xxxx xxxx"
                                    value="<?= htmlspecialchars($settings['gmail_smtp_pass'] ?? '') ?>">
                                <p class="form-help">Generate in Google Account &gt; Security &gt; 2-Step Verification
                                    &gt; App Passwords.</p>
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingAlertEmail">Recipient Email Address</label>
                                <input type="email" id="settingAlertEmail" class="form-control"
                                    placeholder="alerts@yourdomain.com"
                                    value="<?= htmlspecialchars($settings['alert_email_to'] ?? '') ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 2rem;">
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <p class="form-help">Current Server Time: <strong><?= date('Y-m-d H:i:s T') ?></strong>
                                </p>
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingDefaultInterval">Default Check Interval
                                    (Minutes)</label>
                                <input type="number" id="settingDefaultInterval" class="form-control" min="1"
                                    value="<?= (int) ($settings['default_interval_mins'] ?? 15) ?>">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingBigChangeThreshold">Large Change Attachment
                                    Threshold (Lines)</label>
                                <input type="number" id="settingBigChangeThreshold" class="form-control" min="5"
                                    max="500" value="<?= (int) ($settings['diff_big_change_threshold_lines'] ?? 30) ?>">
                                <p class="form-help">If changed lines exceed this threshold, separate color-highlighted
                                    HTML files are attached instead of inline text.</p>
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="settingNewPassword">Update Admin Password</label>
                                <input type="password" id="settingNewPassword" class="form-control"
                ...
                <div
                    style="background: var(--bg-subtle); padding: 1.25rem; border-radius: var(--radius-md); border: 1px solid var(--border-color); margin-bottom: 1.5rem;">
                    <h4 style="font-size: 0.95rem; margin-bottom: 0.5rem; color: var(--success);">💾 Single-File Full
                        Backup & Restore</h4>
                    <p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 1rem;">
                        Export everything (all target monitors, settings, notification tokens, stats, and text history
                        snapshot logs) into a single downloadable JSON backup file, or restore your entire configuration
                        in one click.
                    </p>
                    <div style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: center;">
                        <a href="api.php?action=export_backup" class="btn btn-secondary btn-sm" download>⬇️ Download
                            Full Backup (.json)</a>
                ...
                <div
                    style="background: var(--bg-subtle); padding: 1.25rem; border-radius: var(--radius-md); border: 1px solid var(--border-color); margin-bottom: 1.5rem;">
                    <h4 style="font-size: 0.95rem; margin-bottom: 0.5rem; color: var(--info);">🕒 Bluehost cPanel Cron
                        Setup</h4>
                    <p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 0.5rem;">
                        To run automated background checks, add this command to <strong>cPanel &gt; Cron Jobs</strong>
                        (e.g. every 15 minutes):
                    </p>
                    <pre
                        style="background: var(--bg-code); padding: 0.75rem; border-radius: 4px; font-family: var(--font-mono); font-size: 0.8rem; overflow-x: auto; color: var(--info); border: 1px solid var(--border-color);">/usr/local/bin/php <?= htmlspecialchars(CM_ROOT) ?>/public/cron.php</pre>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="monitorName" class="form-control"
                                placeholder="e.g. Competitor Pricing / API Health" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="monitorUrl">Target URL *</label>
                            <input type="url" id="monitorUrl" class="form-control"
                                placeholder="https://example.com/page or /api" required>
                        </div>
                    </div>
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="monitorGroup" class="form-control" list="existingGroupsList"
                                placeholder="e.g. E-Commerce, Competitors, APIs" value="General">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select id="monitorTemplate" class="form-control">
                                <?php foreach ($builtinTemplates as $key => $tmpl): ?>
                                    <option value="<?= htmlspecialchars($key) ?>"><?= htmlspecialchars($tmpl['name']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div
                        style="background: var(--bg-subtle); padding: 1rem; border-radius: var(--radius-sm); border: 1px solid var(--border-color); margin-bottom: 1.25rem;">
                        ...
                        <p style="font-size: 0.75rem; color: var(--text-muted); margin-bottom: 0.75rem;">
                            Send more frequent checks during high-priority hours (e.g. market/business hours) and fewer
                            checks during off-hours.
                        </p>
                        <div id="peakScheduleFields" style="display: none;">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    <label class="form-label" for="monitorPeakInterval">Peak Check Interval
                                        (Mins)</label>
                                    <input type="number" id="monitorPeakInterval" class="form-control" min="1" value="5"
                                        placeholder="e.g. 5">
                                </div>
                                <div class="form-group">
                                    <label class="form-label" for="monitorOffpeakInterval">Off-Peak Check Interval
                                        (Mins)</label>
                                    <input type="number" id="monitorOffpeakInterval" class="form-control" min="1"
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="monitorHeaders" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="monitorCookies" class="form-control"
                            placeholder="session_id=xyz; auth=1">
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <div
                            ...
                            <label class="form-label" for="bulkMonitorsText" style="margin-bottom: 0;">Target List
                                *</label>
                            <span id="bulkLineCount"
                                style="font-size: 0.775rem; color: var(--info); font-weight: 600;">0 targets
                                detected</span>
                        </div>
                        <textarea id="bulkMonitorsText" class="form-control" rows="8"
                            placeholder="https://example.com/page1&#10;My API, https://api.example.com/v1/health&#10;Store Front | https://store.example.com"
                            style="font-family: var(--font-mono); font-size: 0.85rem;" required></textarea>
                    </div>

                    <h4
                        style="font-size: 0.95rem; color: var(--text-primary); margin: 1.25rem 0 0.75rem 0; padding-bottom: 0.35rem; border-bottom: 1px solid var(--border-color);">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="bulkDefaultGroup" class="form-control" list="existingGroupsList"
                                placeholder="e.g. E-Commerce, APIs" value="General">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="number" id="bulkDefaultTimeout" class="form-control" min="5" max="60"
                                value="25">
                        </div>
                    </div>
            ...
            <form id="bulkGroupForm">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="bulkGroupInput" class="form-control" list="existingGroupsList"
                            placeholder="Enter new group or pick existing..." required>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        onclick="closeModal('bulkGroupModal')">Cancel</button>
            ...
            <form id="bulkEditForm">
                <div class="modal-body">
                    <div class="format-helper-box" style="margin-bottom: 1rem;">
                        💡 Check the box next to any setting you want to batch update across <strong
                            id="bulkEditTargetCount">0</strong> selected monitor(s). Unchecked settings will remain
                        untouched.
                    </div>
                    ...
                    <div
                        ...
                        <div id="bulkEditIntervalFields" style="display: none; margin-top: 0.5rem;">
                            <input type="number" id="bulkEditIntervalVal" class="form-control" min="1" value="15"
                    ...
                    <div
                        ...
                        <div id="bulkEditTimeoutFields" style="display: none; margin-top: 0.5rem;">
                            <input type="number" id="bulkEditTimeoutVal" class="form-control" min="5" max="60"
                                value="25" placeholder="Timeout in seconds">
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div id="bulkEditPeakFields" style="display: none; margin-top: 0.5rem;">
                            ...
                            <div id="bulkEditPeakConfigBox">
                                <div class="form-grid">
                                    <div class="form-group">
                                        <label class="form-label" for="bulkEditPeakStartVal">Peak Start Hour
                                            (24h)</label>
                                        <select id="bulkEditPeakStartVal" class="form-control">
                                            <?php for ($h = 0; $h < 24; $h++): ?>
                                                <option value="<?= $h ?>" <?= $h === 9 ? 'selected' : '' ?>>
                                                    <?= sprintf('%02d:00 (%s)', $h, date('g A', strtotime("$h:00"))) ?>
                                                </option>
                                            <?php endfor; ?>
                                        </select>
                                    </div>
                                ...
                                <div class="form-grid">
                                    <div class="form-group">
                                        <label class="form-label" for="bulkEditPeakIntervalVal">Peak Interval
                                            (Mins)</label>
                                        <input type="number" id="bulkEditPeakIntervalVal" class="form-control" min="1"
                                            value="5" placeholder="e.g. 5 mins">
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label" for="bulkEditOffpeakIntervalVal">Off-Peak Interval
                                            (Mins)</label>
                                        <input type="number" id="bulkEditOffpeakIntervalVal" class="form-control"
                                            min="1" value="60" placeholder="e.g. 60 mins">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        ...
                        <label class="checkbox-label"
                            style="font-weight: 600; color: var(--text-primary); margin-bottom: 0.5rem; min-height: auto;">
                            <input type="checkbox" id="bulkEditApplyNotifications"> Update Notification & Strip Tags
                            Settings
                        </label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        onclick="closeModal('bulkEditModal')">Cancel</button>
    ...
    <div class="modal-backdrop" id="historyModal">
        <div class="modal-dialog" style="max-width: 960px;">
            ...
            <div class="modal-body">
                <!-- Top Actions Bar -->
                <div
                    style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem; background: var(--bg-subtle); padding: 0.85rem 1rem; border-radius: var(--radius-md); border: 1px solid var(--border-color); flex-wrap: wrap; gap: 0.75rem;">
                    <div>
                        <div id="historyLogMeta"
                            style="font-size: 0.85rem; font-weight: 600; color: var(--text-primary);"></div>
                        <div style="font-size: 0.75rem; color: var(--text-muted);">Timestamped execution events & change
                            history</div>
                    </div>
                ...
                <div
                    style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem;">
                    <div style="display: flex; gap: 0.5rem;" id="historyViewModeBtns">
                        <button type="button" class="btn btn-primary btn-sm" id="btnHistoryModeDiff">🔴/🟢 Red & Green
                            Diff</button>
                        <button type="button" class="btn btn-secondary btn-sm" id="btnHistoryModeRaw">📄 Raw
                            Content</button>
                        <button type="button" class="btn btn-secondary btn-sm" id="btnHistoryModeChanges">📋 Change Log
                            (<span id="historyChangesCount">0</span>)</button>
                    </div>

                    <!-- Comparison Selectors (Diff Mode) -->
                    <div id="historyDiffControls"
                        style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                        <label style="font-size: 0.8rem; color: var(--text-muted);">Compare:</label>
                        <select id="diffVersionOld" class="form-control"
                        ...
                        <select id="diffVersionNew" class="form-control"
                            style="min-height: 32px; padding: 0.2rem 0.5rem; font-size: 0.8rem; width: auto;">
                            <option value="">Latest Captured</option>
                        </select>
                        <button type="button" class="btn btn-secondary btn-sm" id="btnComputeHistoryDiff"
                            style="padding: 0.25rem 0.6rem;">⚡ Diff</button>
                    </div>
                ...
                <div id="historyDiffViewContainer"
                    style="max-height: 460px; overflow-y: auto; border: 1px solid var(--border-color); border-radius: var(--radius-md); background: var(--bg-code); padding: 0.75rem;">
                        ...
                        <tbody id="historyChangesTableBody">
                            <tr>
                                <td colspan="4" style="text-align: center; color: var(--text-muted); padding: 1.5rem;">
                                    No detected changes recorded yet.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <footer class="app-footer"
        ...
        <div
            ...
            <label for="themeSelect" style="font-size: 0.85rem; color: var(--text-muted); cursor: pointer;"
                title="Change UI Theme">🎨 Theme:</label>
            <select id="themeSelect" class="form-control"
                style="min-height: 32px; padding: 0.2rem 0.5rem; font-size: 0.825rem; width: auto; background: transparent; border: none; color: var(--text-primary); cursor: pointer;"
    ...
    <div class="modal-backdrop" id="changesExplorerModal">
        <div class="modal-dialog" style="max-width: 1040px;">
            ...
            <div class="modal-body">
                <!-- Filter Bar -->
                <div
                    style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem; background: var(--bg-subtle); padding: 0.85rem 1rem; border-radius: var(--radius-md); border: 1px solid var(--border-color);">
                    <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; align-items: center; flex: 1;">
                        <input type="text" id="changesSearchInput" class="form-control"
                            placeholder="🔍 Search target name or group..."
                            style="min-height: 36px; min-width: 200px; flex: 1;">
                        <input type="date" id="changesDateInput" class="form-control" title="Filter changes by date"
                            style="min-height: 36px; width: 145px;">
                    ...
                    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                        <button type="button" class="btn btn-secondary btn-sm" id="changesRefreshBtn">🔄
                            Refresh</button>
                        <button type="button" class="btn btn-secondary btn-sm" id="changesArchiveSelectedBtn"
                            style="display: none;">📦 Archive Selected (<span
                                id="changesSelectedCount">0</span>)</button>
                        <button type="button" class="btn btn-secondary btn-sm" id="changesDeleteSelectedBtn"
                            style="display: none;">🗑️ Delete Selected</button>
                        <button type="button" class="btn btn-danger btn-sm" id="changesClearAllBtn">🔥 Clear All
                            Changes</button>
                    </div>
                </div>
                ...
                <div class="table-responsive" style="max-height: 500px; overflow-y: auto;">
                    <table class="data-table" id="allChangesTable">
                        <thead>
                            <tr>
                                <th style="width: 40px;" class="th-checkbox">
                                    <input type="checkbox" id="selectAllChangesCheckbox" class="custom-checkbox"
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary"
                    onclick="closeModal('changesExplorerModal')">Close</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-backdrop" id="changeDetailModal">
        <div class="modal-dialog" style="max-width: 960px;">
            ...
            <div class="modal-body">
                <div
                    style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; background: var(--bg-subtle); padding: 0.75rem 1rem; border-radius: var(--radius-md); border: 1px solid var(--border-color); flex-wrap: wrap; gap: 0.5rem;">
                    <div id="changeDetailMeta" style="font-size: 0.85rem;"></div>
                    <div style="display: flex; gap: 0.5rem;">
                        <button type="button" class="btn btn-secondary btn-sm" id="changeDetailArchiveBtn">📦
                            Archive</button>
                        <button type="button" class="btn btn-danger btn-sm" id="changeDetailDeleteBtn">🗑️
                            Delete</button>
                    </div>
                </div>

                <div id="changeDetailDiffContainer"
                    style="max-height: 480px; overflow-y: auto; border: 1px solid var(--border-color); border-radius: var(--radius-md); background: var(--bg-code); padding: 0.75rem;">
